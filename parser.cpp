...
{ 
  //public:
    ...
    /* union kind{ StmtKind stmt; ExpKind exp; kind(){} ~kind(){}} kind;
    union attr{ string op;
            int val;
            string name; 
            attr(){}
            ~attr(){}
...
{ 
  ...
  if(token.second.compare("ASSIGN") == 0)
  {
    printf("BREAKPOINT");
    printf("BREAKPOINT");
    printf("BREAKPOINT");
...
TreeNode * stmt_sequence(void)
{
  ...
  TreeNode * p = t;
  while (token.second.compare("END") != 0 && token.second.compare("ELSE") != 0 && token.second.compare("UNTIL")!= 0 && tokens.empty() == 0)
  {
    TreeNode * q;
    match("SEMICOLON");
    q = statement();
