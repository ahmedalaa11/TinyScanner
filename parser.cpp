...
TreeNode * stmt_sequence(void)
{
  ...
  TreeNode * p = t;
  while (token.second.compare("END") != 0 && token.second.compare("ELSE") != 0 && token.second.compare("UNTIL")!= 0 && endFile == 0)
  {
    TreeNode * q;
    match("SEMICOLON");
    q = statement();
...
void printToken( string token, const char* tokenString )
{ 
  if(token.compare("IF")== 0 || (token.compare("THEN")== 0)||
     token.compare("ELSE")== 0 || (token.compare("END")== 0)||
     token.compare("REPEAT")== 0 || (token.compare("UNTIL")== 0)||
     token.compare("READ")== 0 || (token.compare("WRITE")== 0))
    {fprintf(listing, "reserved word: %s\n",tokenString);}
    else if (token.compare("ASSIGN")== 0)fprintf(listing,":=\n");
    else if (token.compare("LESSTHAN")== 0)fprintf(listing,"<\n");
    else if (token.compare("EQUAL")== 0)fprintf(listing,"=\n");
    else if (token.compare("OPENBRACKET")== 0)fprintf(listing,"(\n");
    else if (token.compare("CLOSEDBRACKET")== 0) fprintf(listing,")\n");
    else if (token.compare("SEMICOLON")== 0)fprintf(listing,";\n");
    else if (token.compare("PLUS")== 0)fprintf(listing,"+\n");
    else if (token.compare("MINUS")== 0)fprintf(listing,"-\n");
    else if (token.compare("MUL")== 0)fprintf(listing,"*\n");
    else if (token.compare("DIV")== 0)fprintf(listing,"/\n");
    else if (token.compare("NUMBER")== 0)fprintf(listing,"NUMBER, val= %s\n",tokenString);
    else if (token.compare("IDENTIFIER")== 0)fprintf(listing,"IDENTIFIER, name= %s\n",tokenString);
    else if (token.compare("NUMBER")== 0)fprintf(listing,"NUMBER, val= %s\n",tokenString);
    else fprintf(listing,"Unknown token: %d\n",token);
}
